...
//!
//! The path + doc helpers are `pub(crate)` so provider activation (which writes
//! the `env` block) can reuse them.
...
pub(crate) fn settings_path(scope: &ScopeRef, layer: &str, home: &Path) -> Result<PathBuf, String> {
...
/// Read a settings doc (whole JSON object), or `{}` if absent.
pub(crate) fn read_doc(path: &Path) -> Result<Value, String> {
...
/// Write a settings doc (pretty JSON + trailing newline).
pub(crate) fn write_doc(path: &Path, value: &Value) -> Result<(), String> {
...
pub fn read_settings(scope: ScopeRef, layer: String) -> Result<Value, String> {
    read_doc(&settings_path(&scope, &layer, &home_dir()?)?)
}
...
pub fn write_settings(scope: ScopeRef, layer: String, value: Value) -> Result<(), String> {
    ...
    write_doc(&settings_path(&scope, &layer, &home_dir()?)?, &value)
}
